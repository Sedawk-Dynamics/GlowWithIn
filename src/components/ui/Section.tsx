...
export function SectionHeading({
  ...
  return (
    <div className={`${centred ? "mx-auto max-w-[820px] text-center" : "max-w-[820px] text-left"} ${className}`}>
      {eyebrow && <p className="gw-eyebrow">{tm(eyebrow)}</p>}
      <Tag className={Tag === "h1" ? "gw-h1" : Tag === "h2" ? "gw-h2" : "gw-h3"}>{tm(title)}</Tag>
      <hr className={`gw-rule ${centred ? "gw-rule--center" : ""}`} />
      {lead && <div className="gw-lead">{tm(lead)}</div>}
    </div>
  );
}
...
export function Signature({ line, signOff, light = false }: { line: string; signOff?: string; light?: boolean }) {
  return (
    <div className="text-center">
      <p className={`m-0 font-serif text-[clamp(26px,3.4vw,40px)] leading-[1.2] ${light ? "text-white" : "text-ink"}`}>{tm(line)}</p>
      {signOff && (
        <p className={`mt-3 mb-0 font-sans text-[13px] font-semibold uppercase tracking-[0.2em] ${light ? "text-white/80" : "text-mocha"}`}>{tm(signOff)}</p>
      )}
    </div>
  );
}
